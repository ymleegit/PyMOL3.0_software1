...
def allints(*X):
   return reduce(op.and_, (type(x)==int for x in X), True)

def allfloats(*X):
   return reduce(op.and_, (type(x)==float for x in X), True)

def allnums(*X):
   return reduce(op.and_, (isint(x) or isfloat(x) for x in X), True)

def allpoints(*X):
   return reduce(op.and_, (type(x)==Point for x in X), True)

def allvecs(*X):
   return reduce(op.and_, (type(x)==Vec for x in X), True)

def allvorpts(*X):
   return reduce(op.and_, (isvec(x) or ispoint(x) for x in X), True)

def alllines(*X):
   return reduce(op.and_, (type(x)==Line for x in X), True)

def allplanes(*X):
   return reduce(op.and_, (type(x)==Plane for x in X), True)

def allmats(*X):
   return reduce(op.and_, (type(x)==Mat for x in X), True)

def allxforms(*X):
   return reduce(op.and_, (type(x)==Xform for x in X), True)

def alllists(*X):
   return reduce(op.and_, (type(x)==list for x in X), True)

def alltuples(*X):
   return reduce(op.and_, (type(x)==tuple for x in X), True)

def alliters(*X):
   return reduce(op.and_, (hasattr(x, "__iter__") for x in X), True)

def anyints(*X):
   return reduce(op.or_, (type(x)==int for x in X), False)

def anyfloats(*X):
   return reduce(op.or_, (type(x)==float for x in X), False)

def anynums(*X):
   return reduce(op.or_, (isint(x) or isfloat(x) for x in X), False)

def anypoints(*X):
   return reduce(op.or_, (type(x)==Point for x in X), False)

def anyvecs(*X):
   return reduce(op.or_, (type(x)==Vec for x in X), False)

def anyvorpts(*X):
   return reduce(op.or_, (isvec(x) or ispoint(x) for x in X), False)

def anylines(*X):
   return reduce(op.or_, (type(x)==Line for x in X), False)

def anyplanes(*X):
   return reduce(op.or_, (type(x)==Plane for x in X), False)

def anymats(*X):
   return reduce(op.or_, (type(x)==Mat for x in X), False)

def anyxforms(*X):
   return reduce(op.or_, (type(x)==Xform for x in X), False)

def anylists(*X):
   return reduce(op.or_, (type(x)==list for x in X), False)

def anytuples(*X):
   return reduce(op.or_, (type(x)==tuple for x in X), False)
...
class Point(object):
   ...
   def __eq__(self, other):
      return (type(self)==type(other) and abs(self.x - other.x) < EPS
...
def dihedral(p1, p2, p3, p4=None):
   ...
   if allpoints(p1, p2, p3, p4):
      ...
      y = a.dot(b.cross(c))
      return atan2(y, x)
   if allvecs(p1, p2, p3) and p4==None:
...
class Plane(object):
   ...
   def __init__(self, a, b=None, c=None):
      if isplane(a):
         a, b = a.n, a.p
      elif isline(a) and ispoint(b) and c==None:
         a, b = a.d.cross(a.p - b), b
      elif isline(a) and isvec(b) and c==None:
         a, b = a.d.cross(b), a.p
      if allpoints(a, b, c):
         a, b = (a - b).cross(a - c), a
...
#       elif yx==None and isvec(xx) and isvec(xy) and isvec(xz):
...
# def randrot(n=1):
#    if n==1: return rotation_matrix_degrees(randvec(),uniform(0,1)*360)
...
#       if isvec(R) and t==None: R,t = Imat,R
...
# def randxform(n=1):
#    if n==1: return Xform(randrot(),randvec())
